...
const BoardAnimation = (props: BoardProps) => {
  ...
  props.marbleList.forEach((marble: Marble) => {
    ...
    if (marble.position < 0) {
      ...
      homeOccupation[marble.position] = marble;
    } else if (marble.position >= 1000) {
      // positions above 1000 correspond to houses
      houseOccupation[marble.position] = marble;
    } else {
      stepOccupation[marble.position] = marble;
    }
  });

  // make the step radius smaller for the 6 player board
        ...
        >
          ...
          {boardData.homes.map((data) => (
            <circle
              ...
              className={
                homeOccupation[data.id]
                  ? "step occupied occupied-" +
                    homeOccupation[data.id].color
                  : "step"
              }
          ...
          {boardData.houses.map((data) => (
            <circle
              ...
              className={
                houseOccupation[data.id]
                  ? "step occupied occupied-" +
                    houseOccupation[data.id].color
                  : "step"
              }
